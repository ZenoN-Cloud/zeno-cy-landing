...
export async function POST(request: Request) {
  try {
    ...
    const { email, companySize, banks, pain } = payload;
    
    console.log("✅ NEW BETA REQUEST:", payload);
    
    if (process.env.NOTION_TOKEN && process.env.NOTION_DATABASE_ID) {
      await sendToNotion({ email, companySize, banks, featureRequests: pain });
...
async function sendToNotion(data: { email: string; companySize: string; banks: string[]; featureRequests?: string }) {
